...
//#import "vpn-tunnel-Swift.h"

@import NetworkExtension;

NS_ASSUME_NONNULL_BEGIN

NSString *const appGroup = @"group.com.simform.layerdemoapp";
...
+ (void) preflightCheck {
    @autoreleasepool {
        ...
        if(![fileManager fileExistsAtPath:[[containerURL path] stringByAppendingPathComponent:@"dnscrypt/dnscrypt.toml"]]) {
            NSString *queryLogPath = [[containerURL path] stringByAppendingPathComponent:@"/dnscrypt/logs/query.log"];
            NSString *blackListPath = [[containerURL path] stringByAppendingPathComponent:@"/dnscrypt/blacklist.txt"];
            NSString *blackListLogFile = [[containerURL path] stringByAppendingPathComponent:@"/dnscrypt/logs/blocked.log"];
            NSString *str = [NSString stringWithFormat:@"listen_addresses = [\"127.0.0.1:53\", \"[::1]:53\"]\n"
                             ...
                             "[query_log]\nfile = \"%@\"\n"
                             "format = \"tsv\"\n",
                             blackListLogFile, blackListPath, [[containerURL path] stringByAppendingPathComponent:@"dnscrypt/resolvers/public-resolvers.md"], queryLogPath];
        ...
        if([fileManager fileExistsAtPath:[[containerURL path] stringByAppendingPathComponent:@"dnscrypt/blacklist.txt"]]) {
